...
export default function CustomCakeOrder() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.releaseDate || !formData.customerName || !formData.email) {
      alert("Please complete required fields.");
      return;
    }
    setIsSubmitting(true);
    try {
      const res = await apiService.createCustomOrder(formData);
      // Accept {success:true, data:{orderId}} or {orderId}
      const newRequestId = res?.data?.orderId || res?.orderId || res?.id || "";
  ...
  const fileUploadHoverStyle = { borderColor: "#ff6f61" };
  const btnStyle = {
    padding: "12px 20px",
    fontSize: "16px",
    border: "none",
  ...
  const btnResetHoverStyle = { background: "#626d6e" };
  const formActionsStyle = { textAlign: "center", marginTop: "20px" };
  ...
  const highlightStyle = { color: "#ff6f61", fontWeight: "bold" };
  ...
  const handleMouseEnter = (key) =>
    setHoverStates((p) => ({ ...p, [key]: true }));
  const handleMouseLeave = (key) =>
    setHoverStates((p) => ({ ...p, [key]: false }));

  const getNavLinkStyle = (i) => ({
    ...navLinkStyle,
    ...(hoverStates[`navLink${i}`] && navLinkHoverStyle),
    ...(i === 1 && navLinkHoverStyle), // active
  });
      ...
      <div style={mainContentStyle}>
        ...
        >
          ...
          <div style={formSectionStyle}>
            ...
            <div style={formGroupStyle}>
              <label style={labelStyle}>Design Image (optional):</label>
              <div
                role="button"
                onClick={handleFileUploadClick}
                onMouseEnter={() => handleMouseEnter("fileUpload")}
                onMouseLeave={() => handleMouseLeave("fileUpload")}
                style={getFileUploadStyle()}
              >
