...
import logo from "../../assets/PDFHeader.png"; // ✅ optional header logo
import { ComplaintsAPI } from "../../services/api"; // adjust if needed
...
export default function ComplaintManagement()
{
  ...
  const sx = {
    ...
    tableWrap: { overflowX: "auto", maxHeight: "500px", overflowY: "auto" },
    ...
    deleteBtnHover: { background: "#c0392b" },
    searchContainer: {
      background: "#fff",
      padding: 20,
      borderRadius: 10,
    ...
    resultsCount: { fontSize: 14, color: "#666", fontStyle: "italic" },
    error: { background: "#fdecea", color: "#c0392b", padding: 12, borderRadius: 8 },
  };
  ...
  async function handleDeleteComplaint(id, complainerName)
  {
    const confirmed = window.confirm(
      `Are you sure you want to delete the complaint by ${complainerName}?`
    );
    if (!confirmed) return;
    try
    {
  ...
  const generatePDF = () =>
  {
    ...
    doc.setFontSize(18);
    doc.text("Complaint Management Report", 70, 20);
    doc.setFontSize(12);
    doc.text(`Generated on: ${date}`, 70, 28);

    const tableColumn = ["Name", "Email", "Complaint", "Type", "Priority", "Status"];
              ...
              <tbody>
                {(filteredComplaints || []).map((c) => (
                  <tr key={c._id}>
                    ...
                    <td style={sx.thtd}>{c.complaintType || "N/A"}</td>
                    <td style={sx.thtd}>
                      <select
                        style={{
                          ...sx.select,
                      ...
                      <select
                        ...
                        onChange={async (e) =>
                        {
                          ...
                          try
                          {
                            ...
                            await ComplaintsAPI.updateAdminType(c._id, newType);
                            setComplaints((prev) =>
                              prev.map((comp) =>
                                comp._id === c._id ? { ...comp, adminType: newType } : comp
                              )
                            );
                          } catch (err)
                          {
                            alert(err.message || "Failed to update admin type");
                          }
                        }}
                      >
                        {["Finance", "Order & Delivery", "User Management", "Product"].map((type) => (
                          <option key={type} value={type}>
                            {type}
                          </option>
                        ))}
